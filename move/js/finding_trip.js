$(document).ready(function(e) {
    var imgObj = document.getElementsByClassName("trip-img");
    var imgLen = imgObj.length;
    var windowWidth = $(window).width();
    $(".trip-img").bind("click",function(event){

    });
    int = setInterval(carouselImg,3000);
    for(var i=0;i<imgLen;i++){
        $(".trip-img").eq(i).css({"top":"0","left":i*windowWidth});
...
function touchmove(event){
    ...
    if( event.targetTouches.length == 1 )
    {
        ...
        var $car_img = $(".trip-img"),
            $this = $(this);
        var thisIndex = $this.index(),
            imgLen = $(".trip-img").length;
        for(var i=0;i < imgLen;i++){
